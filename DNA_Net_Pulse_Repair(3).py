...
class DNANetPulseRepair:
    ...
    def __init__(
        ...
        try:
            ...
            print("✓ تم التحميل من المسار المحلي")
        except Exception as e:
            print(f"⚠ فشل التحميل المحلي: {e}")
            print("→ محاولة التحميل من Hugging Face...")
            try:
                self.controlnet = ControlNetUnionModel.from_pretrained(
                    "xinsir/controlnet-union-sdxl-1.0",   # أضف -promax لو عندك النسخة الـ promax
                    torch_dtype=self.dtype,
                    use_safetensors=True,
                )
                print("✓ تم التحميل من HF")
            except Exception as hf_err:
                raise RuntimeError(f"فشل تحميل ControlNet Union كليًا: {hf_err}")

        self.controlnet = self.controlnet.to(self.device)
        print(f"ControlNet على: {next(self.controlnet.parameters()).device}")
        ...
        try:
            print("  محاولة تحميل StableDiffusionXLControlNetInpaintPipeline...")
            pipe = StableDiffusionXLControlNetInpaintPipeline.from_pretrained(
                ...
                variant="fp16",
                use_safetensors=True,
                safety_checker=None,              # يسكت تحذير الـ safety_checker
            )
            print("  → تم تحميل النموذج الأساسي")

            # ─── لا تنقل للجهاز يدويًا هنا إذا كنت هتستخدم offload ───
            # pipe.to(self.device)   ← شيل هذا السطر تمامًا أو علّقه

            if self.device.type == "cuda":
                print("  تفعيل الـ offloading والتحسينات...")

                # أولاً: offload (يجب يكون قبل أي نقل يدوي)
                try:
                    pipe.enable_model_cpu_offload()
                    print("  → model_cpu_offload ناجح")
                except Exception as off_e:
                    print(f"  !! فشل model_cpu_offload: {off_e}")
                    try:
                        pipe.enable_sequential_cpu_offload()
                        print("  → sequential_cpu_offload ناجح (بديل أقوى)")
                    except Exception as seq_e:
                        print(f"  !! فشل sequential أيضًا: {seq_e}")
                        print("  → سيتم التشغيل بدون offload (قد يحتاج VRAM كبير)")

                # ثانيًا: الـ slicing (الطريقة الحديثة المفضلة)
                try:
                    pipe.vae.enable_slicing()                    # الطريقة الجديدة (بدل enable_vae_slicing)
                    pipe.enable_attention_slicing(slice_size="max")  # أو 1 لو الـ VRAM ضيق جدًا
                    print("  → vae slicing + attention slicing مفعّل")
                except Exception as s_e:
                    print(f"  !! فشل في تفعيل slicing: {s_e}")

            # لو مش هتستخدم offload على الإطلاق (اختياري للـ debug فقط)
            # else:
            #     pipe.to(self.device)
            #     print("  → نقل يدوي إلى cuda (بدون offload)")

            self.pipeline = pipe
            print("✓ Pipeline تم تحميله بنجاح 100%")

        except Exception as e:
            print("\n" + "="*80)
            print("فشل كامل في تحميل الـ PIPELINE !!")
            print(f"نوع الخطأ  : {type(e).__name__}")
            print(f"التفاصيل   : {str(e)}")
            print("="*80 + "\n")
            self.pipeline = None
            # raise  # ← شيل التعليق لو عايز يوقف البرنامج فورًا عند الفشل
    ...
    def create_dna_pulse_repair_layer(
        ...
        img: Image.Image,
        pulse_steps: int = 6,
        positive_pulse: Tuple[int, int, int] = (30, 25, 20),
        negative_pulse: Tuple[int, int, int] = (-10, -8, -5),
        ...
        """
        إنشاء طبقة نبضية DNA-inspired مع كشف المناطق السالبة ودمج مباشر.
        ترجع الصورة المُصلحة (repaired).
        """
        ...
        # 1. كشف المناطق المنهارة/السالبة
        ...
        # 1/1. خريطة التصور الملونة (سالب/موجب) – اختياري تمامًا
        signed_map = None
        if debug_save:
            ...
            signed_map.save("debug_signed_map.jpg")
            print("   → تم حفظ debug_signed_map.jpg لمعاينة المناطق السالبة/الموجبة")

        # 2. خريطة التصور (اختياري)
        if debug_save:
            signed_map = self.detect_dead_zones(
                img,
                method="multi",
            ...
            signed_map.save("debug_signed_map.jpg")
            print("   → تم حفظ debug_signed_map.jpg للمعاينة")
        ...
        for step in range(pulse_steps):
            ...
            # موجبة
            r = int(positive_pulse[0] * (1 + step * 0.08))
            g = int(positive_pulse[1] * (1 + step * 0.08))
            b = int(positive_pulse[2] * (1 + step * 0.08))
            pos_fill = (r, g, b, alpha)
            draw.rectangle((0, 0, size[0], size[1]), fill=pos_fill)

            # سالبة
            r = int(negative_pulse[0] * (1 - step * 0.1))
            g = int(negative_pulse[1] * (1 - step * 0.1))
            b = int(negative_pulse[2] * (1 - step * 0.1))
            neg_fill = (r, g, b, alpha // 2)
        ...
        # 5. دمج الطبقة على المناطق السالبة فقط
        repaired = Image.composite(layer, img.convert("RGBA"), dead_mask.convert("L"))
    ...
    def add_dna_colored_layers(
        ...
        mask: Image.Image,
        base_colors: List[Tuple[int, int, int]] = [(255, 80, 80), (80, 255, 80), (80, 80, 255)],  # ← غيّر هنا
        ...
        if blend_mode == "density":
            ...
            for i, color in enumerate(base_colors):   # ← هنا base_colors
                layer[..., :3] += (np.array(color, dtype=np.uint16) * weights[..., i, None]).astype(np.uint8)

            layer[..., 3] = (mask_arr * 255 * opacity).astype(np.uint8)   # ← هنا تم التصحيح
        ...
        # ضمان عدم التجاوز (اختياري لكن مفيد)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # هنا نحدد النوع صراحةً عشان الـ type checker يرتاح
        seed1: Tuple[int, int, int] = color_engine.generate_dnd_seed_color("Fire")
        seed2: Tuple[int, int, int] = color_engine.generate_dnd_seed_color("Ice")

        print(f"Seed 1 (Fire): {seed1}  → {DND_COLOR_MENU['Fire']['hex']}")
        print(f"Seed 2 (Ice):  {seed2}  → {DND_COLOR_MENU['Ice']['hex']}")

        mixed: Tuple[int, int, int] = color_engine.mix_dnd_seed_colors(seed1, seed2, ratio=0.45)
